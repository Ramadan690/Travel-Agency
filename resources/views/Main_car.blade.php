...
<li class="nav-item"><a class="nav-link" href="{{ route('logout') }}">
        ...
        <!-- About section-->
        <section id="about" style="background: url('https://wallpaperaccess.com/full/31189.jpg');">
            <div class="container px-4" style="background: url('https://wallpaperaccess.com/full/31189.jpg');">
...
<div class="input-group">
  ...
  <select class="form-select" id="Company" >
    <option value="<?php if (isset($_GET['Company'])) {echo $_GET['Company'];}else{}?>">-<?php if (isset($_GET['Company'])) {echo $_GET['Company'];}else{}?>-</option>
...
<button type="button" class="btn btn-light" onclick="window.location.href = 'http://localhost:8000/CarRental'" style="margin-left: 10px;border:1px solid black;border-radius: 10px;">Clear Filter</button>
...
<div class="input-group mb-3">
 <select class="form-select" id="Fuel">
    <option value="<?php if (isset($_GET['Type'])) {echo $_GET['Type'];}else{}?>" selected>-<?php if (isset($_GET['Type'])) {echo $_GET['Type'];}else{echo "Fuel"; }?>-</option>
...
<div class="card" style="margin: 20px;">
  ...
  <div class="card-body">
    <h6 class="card-title">
      <B>{{$event->CarModel}}</B> ({{$event->Reference}})</h6>

    <h5 style="float: right;">Price : <B style="color: red">RM {{$event->price}}</B>/day</h5>
...
<td>{{$event->CarModel}}</td>
<td><b>{{$event->Fuel}}</b></td>
<td>{{$event->Location}}</td>
<td>{{$event->Date}}</td>
<td><b>{{$event->Company }}</b></td>
...
<div class="modal fade" id="seat{{$event->Reference}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-fullscreen">
   ...
   <h5>Car Model : <b>{{$event->CarModel}}</b></h5>
    
<hr>
Location : {{$event->Location}}
<hr>
Price : <B style="color: red">RM {{$event->price}}</B>/day
<hr>
...
<form method="POST" id="Booking" onsubmit="return Booking{{$event->Reference}}(event)">
{{csrf_field()}}

<div class="mb-3">
  <input type="text" class="form-control" name="ref" id="ref" placeholder="" value="" readonly></div>
...
<div class="mb-3">
  <label for="Arrival" class="form-label">Pickup</label>
    <input type="Date" class="form-control" id="Arrival" name="Arrival" placeholder="">
</div>

<div class="mb-3">
  <label for="Departure" class="form-label">Return</label>
...
$.ajaxSetup({
  ...
  $.ajax(
    {
        url: '{{route("CarBooking")}}',
        type: 'get',
        data: {
            "CarModel":'<?php echo $event->CarModel; ?>',
            "name": name,
            "Email": Email,
            "DOB": DOB,
            "Arrival":Arrival,
...
var Cvalue = company.value;

var available = document.getElementById("Available");
var Avalue = available.value;

var fuel = document.getElementById("Fuel");
var Fvalue = fuel.value;
...
count += "&MIprice="+MIvalue;
 ...
 window.location.href = 'http://localhost:8000/CarRental?'+count;
